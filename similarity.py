...
# BJ
import Blackjack.env as BJ_env
import Blackjack.agent as BJ_agent
import Blackjack.HXP_tools as BJ_HXP_tools
            ...
            for idx, line in enumerate(str_history):
                ...
                if line[0] == '(':
                    if PROBLEM == 'FL':
                        ...
                        a, b = [int(t) for t in line[1:-1].split(', ')[:-1]]
                        #print(a,b)
                        c = line[1:-1].split(', ')[-1] == 'True'
                        #print(c)
                        tmp_row = tuple([a,b,c])
...
if __name__ == "__main__":
    ...
    if PROBLEM == 'FL':
        ...
        agent_Q_dirpath = "Q-tables" + os.sep + "Agent"
        #  Env initialization
        env = BJ_env.MyBlackjack()
        #  Agent initialization
        agent = BJ_agent.Agent(POLICY, env)
        #  Load Q table
        agent.load(agent_Q_dirpath)
        # Initialize HXP class
        predicates = {'win': BJ_HXP_tools.win,
                      'lose': BJ_HXP_tools.lose}
        functions = [BJ_HXP_tools.transition,
                     BJ_HXP_tools.terminal,
                     BJ_HXP_tools.constraint,
                     BJ_HXP_tools.sample,
                     BJ_HXP_tools.preprocess,
                     BJ_HXP_tools.get_actions,
                     BJ_HXP_tools.render]
        add_info = {'select': 'imp', 'agent': agent, 'fixed_horizon': True}
