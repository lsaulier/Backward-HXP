    ...
    def hxp(self, H, predicate, approach, n, imp_type, csv_file, backward, coefs):
        ...
        for i in range(n):
            if imp_type == 'action':
                ...
                hxp.append([(H[current_idx[i] * 2], H[current_idx[i] * 2 + 1], H[current_idx[i] * 2 + 2]), top_idx[i]])
    ...
    def u(self, S, S_not, env, agent, predicate, coefs):
        ...
        for (s, p, r) in S:
            ...
            if isinstance(predicate, str):
                # Deal with probabilistic predicates (e.g. BJ predicates)
                respect = self.predicates[predicate](s, info)
                if respect:
                    r = 1
                    u += p if isinstance(respect, bool) else p * respect
            # Setting: two predicates are studied simultaneously
            else:
                r = [0, 0]
                for i, pred in enumerate(predicate):
                    ...
                    if respect:
                        r[i] = 1
                        u[i] += p if isinstance(respect, bool) else p * respect
        ...
        for sublist in S_not:
            ...
            for (s, p, r) in sublist:
                if isinstance(predicate, str):
                    respect = self.predicates[predicate](s, info)
                    if respect:
                        tmp_u += p if isinstance(respect, bool) else p * respect
                        r = 1
                else:
                    r = [0, 0]
                    for i, pred in enumerate(predicate):
                        ...
                        if respect:
                            r[i] = 1
                            tmp_u[i] += p if isinstance(respect, bool) else p * respect
